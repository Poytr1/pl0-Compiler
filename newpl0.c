...
L:
	...
	{ // symbol is a reserved word or an identifier.
		...
		if (i - 1 > j) {
			...
			if(ch == '['){ // symbol is an array
				while(ch == '['){
					wide++;
					getch();
					getsym();
					arr_temp.dim[wide-1] = num;
					getch();
				}
				arr_temp.n_dim = wide;
				arr_temp.size[wide-1]=1;
				for(i=wide-1;i>0;i--)
					arr_temp.size[i-1] = arr_temp.size[i]*arr_temp.dim[i];
				arr_temp.num = arr_temp.size[0]*arr_temp.dim[0];
				sym = SYM_ARRAY;
			}
			else
...
void enter_arr(int kind)
{
	int i;
	ax++;
	table_arr[ax]=arr_temp;
	strcpy(table_arr[ax].name, id);
	table_arr[ax].kind = kind;
	switch(kind)
	{
		case ID_VARIABLE:
			enter(kind);
			table_arr[ax].low_adr = dx-1;
			for(i=table_arr[ax].num-1;i>0;i--){
				enter(kind);
			}
			table_arr[ax].high_adr = dx-1;
			break;
	}
}
...
void factor(symset fsys)
{
	...
	while (inset(sym, facbegsys))
	{
		...
		else if(sym == SYM_ARRAY)
		{
			...
			{
				for(p=arr_temp.n_dim-1; p>=0; p--)
					i += (arr_temp.size[p]*arr_temp.dim[p]);
				switch (table[i].kind) {
					mask *mk;
					case ID_VARIABLE:
						mk = (mask *) &table[i];
